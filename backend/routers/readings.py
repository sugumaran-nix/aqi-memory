...
async def live_stats():
    ...
    last_scrape = await fetchone(
        "SELECT MAX(scraped_at) AS ts FROM readings"
    )
    return LiveStats(
...
async def export_readings(
    ...
    async def generate():
        header = "timestamp,city,station,pm25,pm10,no2,so2,co,o3,nh3,pb,aqi,dominant_pollutant\n"
        yield header
